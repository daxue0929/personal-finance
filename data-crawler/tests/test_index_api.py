...
# ==================== /api/indexes/dca 定投模拟 ====================

def test_get_dca_simulation(client, index_storage, login, admin_user):
    login(client, admin_user)
    index_storage.get_index_history.return_value = MOCK_HISTORY
    resp = client.get('/api/indexes/dca?index_code=000688&frequency=monthly&amount=1000')

    assert resp.status_code == 200
    data = resp.get_json()
    assert set(data.keys()) >= {'total_invest', 'total_shares', 'market_value',
                                'profit', 'return_rate', 'cost_price', 'schedule', 'lump_sum'}
    # 定投日：1月首(01-02,100)、2月首(02-01,110)
    assert len(data['schedule']) == 2
    assert data['total_invest'] == pytest.approx(2000)
    shares = 1000 / 100 + 1000 / 110
    assert data['total_shares'] == pytest.approx(shares, rel=1e-6)
    # 最新价 115
    assert data['market_value'] == pytest.approx(shares * 115, rel=1e-6)
    # 一次性买入对比：2000 在首日 100 买入 -> 20 份，115 市值 2300，收益 15%
    assert data['lump_sum']['invest'] == pytest.approx(2000)
    assert data['lump_sum']['return_rate'] == pytest.approx(0.15, rel=1e-6)


def test_get_dca_invalid_frequency(client, index_storage, login, admin_user):
    login(client, admin_user)
    index_storage.get_index_history.return_value = MOCK_HISTORY
    resp = client.get('/api/indexes/dca?index_code=000688&frequency=daily&amount=1000')
    assert resp.status_code == 400


def test_get_dca_default_amount(client, index_storage, login, admin_user):
    """amount 缺省默认 1000"""
    login(client, admin_user)
    index_storage.get_index_history.return_value = MOCK_HISTORY
    resp = client.get('/api/indexes/dca?index_code=000688&frequency=monthly')
    assert resp.status_code == 200
    assert resp.get_json()['total_invest'] == pytest.approx(2000)
