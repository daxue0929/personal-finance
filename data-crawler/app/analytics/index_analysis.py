...
def _dca_group_key(d_str: str, frequency: str):
    """定投周期分组键：同一组只取首个交易日买入。"""
    d = datetime.strptime(d_str, '%Y-%m-%d').date()
...
def simulate_dca(dates: List[str], closes: List, frequency: str, amount: float) -> dict:
    """定投模拟。

    :param frequency: 'weekly' / 'biweekly' / 'monthly'
    :param amount: 每期定投金额
    :return: {total_invest, total_shares, market_value, profit, return_rate,
              cost_price, schedule:[{date,close,shares,amount,cum_invest,cum_shares}],
              lump_sum:{invest,shares,market_value,profit,return_rate}}
              lump_sum 为「同总额在首日一次性买入」的对比基准。
    """
    if frequency not in ('weekly', 'biweekly', 'monthly'):
        raise ValueError(f"不支持的定投频率: {frequency}（可选 weekly/biweekly/monthly）")

    empty = {
        'total_invest': 0.0, 'total_shares': 0.0, 'market_value': 0.0,
        'profit': 0.0, 'return_rate': 0.0, 'cost_price': 0.0,
        'schedule': [],
        'lump_sum': {'invest': 0.0, 'shares': 0.0, 'market_value': 0.0,
                     'profit': 0.0, 'return_rate': 0.0}
    }
    if not dates:
        return empty
    ...
    total_shares = 0.0
    for d_str, close in pairs:
        g = _dca_group_key(d_str, frequency)
        if g in seen:
            continue
        seen.add(g)
        price = _f(close)
        ...
        total_shares += shares
        schedule.append({
            'date': d_str, 'close': price, 'shares': shares,
            'amount': amount, 'cum_invest': total_invest, 'cum_shares': total_shares
        })

    latest_close = _f(pairs[-1][1])
    market_value = total_shares * latest_close if total_shares else 0.0
    profit = market_value - total_invest
    return_rate = profit / total_invest if total_invest else 0.0
    cost_price = total_invest / total_shares if total_shares else 0.0

    # 一次性买入对比：定投总金额在首日全仓买入
    first_close = _f(pairs[0][1])
    if total_invest > 0 and first_close > 0:
        lump_shares = total_invest / first_close
        lump_mv = lump_shares * latest_close
        lump_sum = {
            'invest': total_invest, 'shares': lump_shares,
            'market_value': lump_mv, 'profit': lump_mv - total_invest,
            'return_rate': (lump_mv - total_invest) / total_invest
        }
    else:
        lump_sum = empty['lump_sum']

    return {
        'total_invest': total_invest, 'total_shares': total_shares,
        'market_value': market_value, 'profit': profit,
        'return_rate': return_rate, 'cost_price': cost_price,
        'schedule': schedule, 'lump_sum': lump_sum
    }
