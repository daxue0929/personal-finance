...
# 作用：把 Docker Hub 流量走国内镜像（docker.xuanyuan.me / docker.1ms.run /
#      docker.m.daocloud.io 三个 fallback），下载速度 5-10x 提升
...
if [ "$HAS_EXISTING" = "1" ]; then
    # 合并：保留现有字段（log-driver / log-opts / data-root 等），只覆盖/补 registry-mirrors 和并发下载配置
    python3 -c "
import json
